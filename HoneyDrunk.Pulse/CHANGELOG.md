...
- Onboarded Pulse to SonarQube Cloud (ADR-0011 D11). Added `sonar-project.properties` at the inner project subdir and wired a `sonarcloud` job in `pr.yml` that calls `HoneyDrunk.Actions/.github/workflows/job-sonarcloud.yml` after `pr-core`. Sources cover the Pulse + Telemetry families: Pulse.Contracts, Telemetry.Abstractions + OpenTelemetry, six Telemetry Sinks (AzureMonitor, Loki, Mimir, PostHog, Sentry, Shared, Tempo), and Pulse.Collector. Samples (`HoneyDrunk.Pulse.Sample.Api`, `HoneyDrunk.Pulse.Sample.Worker`) excluded from sources. Coverage exclusions: contracts/abstractions (interfaces only). Branch-protection requirement added separately after the first successful run lands.
